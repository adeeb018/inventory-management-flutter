...
/// The type of builder function used by [CustomMenuAnchor.builder] to build the
/// widget that the [CustomMenuAnchor] surrounds.
///
...
/// the menu with.
///
/// The `child` is an optional child supplied as the [CustomMenuAnchor.child]
...
///
/// When creating a menu with [MenuBar] or a [SubmenuButton], a [CustomMenuAnchor] is
...
class _CustomMenuAnchorState extends State<CustomMenuAnchor> {
    ...
    Widget child = OverlayPortal(
      ...
      overlayChildBuilder: (BuildContext context) {
        return _Submenu(
          ...
          addItemBtnTitle: widget.addItemBtnTitle,
          onItemBtnClicked: widget.onItemBtnClicked,
        );
      },
      child: contents,
    );
  ...
  void _closeChildren({bool inDispose = false}) {
    ...
    for (final _CustomMenuAnchorState child
        in List<_CustomMenuAnchorState>.from(
      _anchorChildren,
    )) {
...
/// A controller to manage a menu created by a [MenuBar] or [CustomMenuAnchor].
///
...
class CustomMenuController {
  ...
  ///
  /// If the menu's anchor point (either a [MenuBar] or a [CustomMenuAnchor]) is
...
/// A button for use in a [MenuBar], in a menu created with [CustomMenuAnchor], or on
...
class CustomMenuItemButton extends StatefulWidget {
  ...
  ///
  /// If the menu item button is a descendent of [CustomMenuAnchor] or [MenuBar], then
...
class _CustomMenuItemButtonState extends State<CustomMenuItemButton> {
  ...
  _CustomMenuAnchorState? get _anchor =>
      _CustomMenuAnchorState._maybeOf(context);
  ...
  void _handleFocusChange() {
    if (!_focusNode.hasPrimaryFocus) {
      ...
      _CustomMenuAnchorState._maybeOf(context)?._closeChildren();
    }
  }
  ...
  void _handleSelect() {
    ...
    if (widget.closeOnActivate) {
      _CustomMenuAnchorState._maybeOf(context)?._root._close();
    }
...
class SubmenuButton extends StatefulWidget {
  ...
  /// An optional [CustomMenuController] for this submenu.
  final CustomMenuController? controller;
...
class _SubmenuButtonState extends State<SubmenuButton> {
  ...
  CustomMenuController get _menuController =>
      widget.controller ?? _internalMenuController!;
  _CustomMenuAnchorState? get _parent =>
      _CustomMenuAnchorState._maybeOf(context);
  FocusNode? _internalFocusNode;
  ...
  void didUpdateWidget(SubmenuButton oldWidget) {
    ...
    if (widget.controller != oldWidget.controller) {
      _internalMenuController =
          (oldWidget.controller == null) ? null : CustomMenuController();
    }
  }
...
class _SubmenuDirectionalFocusAction extends DirectionalFocusAction {
  ...
  _CustomMenuAnchorState get _anchor => submenu._menuController._anchor!;
  FocusNode get _buttonFocusNode => submenu._buttonFocusNode;
  _CustomMenuAnchorState? get _parent => _anchor._parent;
...
class _MenuBarAnchor extends CustomMenuAnchor {
  const _MenuBarAnchor({
    ...
    super.clipBehavior,
    super.style,
    required super.addItemBtnTitle,
    required super.onItemBtnClicked,
  });
...
class _MenuBarAnchorState extends _CustomMenuAnchorState {
  ...
  bool get _isOpen {
    ...
    for (final _CustomMenuAnchorState child in _anchorChildren) {
      if (child._isOpen) {
        return true;
      }
    }
...
/// [CustomMenuAnchor] or [MenuBar]. Provided menu items such as [CustomMenuItemButton] and
...
/// [CustomMenuItemButton] or [SubmenuButton]) and renders its child with information
...
/// accelerator markers.
///
/// The built-in menu items [CustomMenuItemButton] and [SubmenuButton] already provide
...
class _MenuAcceleratorLabelState extends State<MenuAcceleratorLabel> {
  ...
  void didChangeDependencies() {
    ...
    _anchor = _CustomMenuAnchorState._maybeOf(context);
  ...
  void _updateAcceleratorShortcut() {
    ...
    if (_showAccelerators &&
      ...
      _shortcutRegistryEntry =
          _shortcutRegistry?.addAll(<ShortcutActivator, Intent>{
        CharacterActivator(
...
/// [SubmenuButton].
///
/// It not only shows the [SubmenuButton.child] or [CustomMenuItemButton.child], but if
