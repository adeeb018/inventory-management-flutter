...
import 'package:inventory_management/shared/presentation/widgets/custom_widgets/custom_menu_anchor.dart';
...
///
/// Used by [CustomDropdownMenu.filterCallback].
typedef FilterCallback<T> = List<CustomDropdownMenuEntry<T>> Function(
  List<CustomDropdownMenuEntry<T>> entries,
  String filter,
);
...
///
/// Used by [CustomDropdownMenu.searchCallback].
typedef SearchCallback<T> = int? Function(
    List<CustomDropdownMenuEntry<T>> entries, String query);
...
class CustomDropdownMenuEntry<T> {
  ...
  ///
  /// The default value is true. If true, the [CustomDropdownMenuEntry.label] will be filled
...
///
/// The menu is composed of a list of [CustomDropdownMenuEntry]s. People can provide information,
...
/// * [CustomDropdownMenuEntry], which is used to build the [MenuItemButton] in the [CustomDropdownMenu] list.
class CustomDropdownMenu<T> extends StatefulWidget {
  ...
  ///
  /// This is a required parameter. It is recommended that at least one [CustomDropdownMenuEntry]
  /// is provided. If this is an empty list, the menu will be empty and only
  /// contain space for padding.
  final List<CustomDropdownMenuEntry<T>> dropdownMenuEntries;
...
class _CustomDropdownMenuState<T> extends State<CustomDropdownMenu<T>> {
  ...
  final CustomMenuController _controller = CustomMenuController();
  bool _enableFilter = false;
  late bool _enableSearch;
  late List<CustomDropdownMenuEntry<T>> filteredEntries;
  ...
  void initState() {
    ...
    _menuHasEnabledItem = filteredEntries.any(
      (CustomDropdownMenuEntry<T> entry) => entry.enabled,
    );
    final int index = filteredEntries.indexWhere(
      (CustomDropdownMenuEntry<T> entry) =>
  ...
  List<CustomDropdownMenuEntry<T>> filter(
    List<CustomDropdownMenuEntry<T>> entries,
  ...
  bool _shouldUpdateCurrentHighlight(List<CustomDropdownMenuEntry<T>> entries) {
  ...
  List<Widget> _buildButtons(
    List<CustomDropdownMenuEntry<T>> filteredEntries,
    TextDirection textDirection, {
    int? focusedIndex,
    bool enableScrollToHighlight = true,
    bool excludeSemantics = false,
    required String addItemBtnTitle,
    required Function()? onItemBtnClicked,
  }) {
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      ...
      //   continue;
      // }
      final CustomDropdownMenuEntry<T> entry = filteredEntries[i];
      ...
      if (entry.enabled && i == focusedIndex) {
        ...
        Color? resolveFocusedColor(
          ...
          return colorStateProperty?.resolve(<MaterialState>{
  ...
  void handlePressed(CustomMenuController controller) {
  ...
  void _handleEditingComplete() {
    if (currentHighlight != null) {
      final CustomDropdownMenuEntry<T> entry =
  ...
  Widget build(BuildContext context) {
    ...
    if (_enableFilter) {
      filteredEntries = widget.filterCallback?.call(
            filteredEntries,
    ...
    Widget menuAnchor = CustomMenuAnchor(
